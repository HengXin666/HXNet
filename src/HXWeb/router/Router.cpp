...
void Router::addController(
    ...
    _routerRadixTree.insert(HX::STL::HXStringUtil::split(path, "/", {requestType}), func);
}
...
Router::EndpointFunc Router::getEndpointFunc(const std::string& requestType, const std::string& path) {
    ...
    if (auto res = _routerRadixTree.find(HX::STL::HXStringUtil::split(pos == std::string::npos ? path : path.substr(0, pos), "/", {requestType})))
        return *res;
    return nullptr;
}
