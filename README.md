...
## 当前分支 `v5.0-uring-throw`

> [!TIP]
> 因为协程是支持异常的, 所以当前分支的协程是可以抛出异常的

- 但是你并不能清晰的把握协程异常是在哪里被处理

- 因此 main 又要重构:
    - 全部协程其返回值用一个 Expected 之类的 协程Task 再包裹一次, 
    - 这样你就需要`co_await co_await fun();`
    - 第一次是fun的返回值, 然后得到的是`Expected<T>`,
    - 第二次`co_await`才是得到返回值;
        - 如果返回值没有问题, 则直接结束`co_await`
        - 如果返回值是`错误`, 则直接中断/退出当前协程!
