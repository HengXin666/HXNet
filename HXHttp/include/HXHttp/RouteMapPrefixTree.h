 ...
 */
template<class T>
class RouteMapPrefixTree : public HXSTL::HXRadixTree<T> {

    std::optional<T> _find(
        const std::vector<std::string>& findLink,
        std::shared_ptr<HXSTL::RadixTreeNode<T>> node,
        std::size_t index
    ) {
    ...
    using HXSTL::HXRadixTree<T>::_root; // 避免找不到父类成员
public:
    explicit RouteMapPrefixTree() : HXSTL::HXRadixTree<T>()
    {}
    ...
    void insert(const std::vector<std::string>& buildLink, const T& val) {
        ...
        for (auto& it : buildLink) {
            ...
            if (findIt == node->child.end()) {
               node = node->child[key] = std::make_shared<HXSTL::RadixTreeNode<T>>();
            } else {
                node = findIt->second;
            }
        }
