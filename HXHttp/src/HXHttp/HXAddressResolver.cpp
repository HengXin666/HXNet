...
int HXAddressResolver::addressInfo::createSocketAndBind() const {
    ...
    int on = 1;
    setsockopt(serverFd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));     // 设置端口复用
...
int HXAddressResolver::addressInfo::createSocket() const {
    /** 注:
...
HXAddressResolver::addressInfo HXAddressResolver::resolve(
